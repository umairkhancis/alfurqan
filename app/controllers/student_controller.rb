...
class StudentController < ApplicationController
  ...
  def profile
    ...
    @father = Guardian.first(:conditions => ["ward_id = ? AND (relation = ? AND relation = ?)", @student.id, 'Father', 'father'])
  ...
  def show2
    @student = Student.find_by_admission_no(params[:id])
    send_data(@student.photo2_data,
      :type => @student.photo2_content_type,
      :filename => @student.photo2_filename,
      :disposition => 'inline')
  end
