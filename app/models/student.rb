...
class Student < ActiveRecord::Base
  ...
  validates_attachment_content_type :photo2, :content_type =>VALID_IMAGE_TYPES,
    :message=>'Image can only be GIF, PNG, JPG',:if=> Proc.new { |p| !p.photo2_file_name.blank? }
  validates_attachment_size :photo2, :less_than => 512000,\
    :message=>'must be less than 500 KB.',:if=> Proc.new { |p| p.photo2_file_name_changed? }
  ...
  def image2_file=(input_data)
    return if input_data.blank?
    self.photo2_filename     = input_data.original_filename
    self.photo2_content_type = input_data.content_type.chomp
    self.photo2_data         = input_data.read
  end
